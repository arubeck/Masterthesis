...
\begin{document}
	...
	\todo[inline]{add reference for biased estimators}
	...
	\todo[inline]{uberleitungs blabla}
	...
	It is a measure of instantaneous potential, that can for example be used to identify a specific model form.
	...
	Two of the main advantages of the parametric survival model are its simplicity and that it is more consistent with the theoretical survival function $S(t)$ than for example nondistributional approaches.
	...
	In contrast to parametric models, it does not require an assumption of the distribution of the outcome.
	An other important advantages is, that it is robust, i.e. it closely approximates the correct parametric model of the survival function.
	
	The model is built upon the following formulation of the hazard function:
	\begin{equation*}
	...
	\todo[inline]{add: when to choose parametric model and when cox model}
	...
	But first, we introduce the main ideas of deep learning and take a closer look at feed forward networks.
	\newpage
	
	\section{Principles of Deep Learning}\label{basicsdl}
	This section relies on \citet*{deeplbook}.
	
	\todo[inline]{allgemeines zu ML}
	...
	\todo[inline]{add that mostly overfitting is a real problem + Quelle}
	...
	\todo[inline]{add: difference of ML and DL}
	...
	\newpage
%	\section{Survival Outcomes} \label{survivalout}
%	In this section we want to use deep learning algorithms, as feedforward networks, for time to event estimation.
%	
%	Standard survival analysis tools, as for example the Kaplan Meyer Method, can not estimate censored data consistently good, because of induced informative censoring.
%	\todo[inline]{explain induces informative censoring}
%	These methods also often rely on restrictive assumptions.
%	Consider for example the cox PH model described in section \ref{cox}, where we had the proportional hazards assumption.
%	
%	We will now try to overcome these limitations by using deep learning to analyze survival data.
	...
	\todo[inline]{explain more detailed, add reference of proof that IPCW trafo is a CUT}
	...
	\todo[inline]{add sth about IPCW weights? add importance of positivity assumption (-> denominator)}
	...
	Y_{DR}^* (\mathcal{O}) = \frac{\Delta Y}{G(Y\vert X)} + \frac{(1-\Delta)m(Y,X;S)}{G(Y \vert X)} - \int_{0}^{Y} \frac{m(c,X;S)}{G(c \vert X)} d\Lambda_G(c \vert X),
	\end{equation}
	...
	The doubly robust CUT in \citet*{drcut}, adapted to our notation, is defined as
	\begin{equation*}
	\hat{Y}_{DR}(\mathcal{O}) = \frac{\Delta Y}{G(Y\vert X)}+ \frac{(1-\Delta)(m(Y,X;S))}{G(Y\vert X)}-\int_{-\infty}^{Y} \frac{m(c,X;S)}{G^2(c\vert X)}dG(c \vert X).
	\end{equation*}
	
	If we can show that $Y^*_{DR}(\mathcal{O})=Y_{DR}(\mathcal{O})$, we can apply theorem \ref{thm:dr} to the transformation described in equation (\ref{eq:drcut}).
	
	\begin{Lemma}
		Under the conditions of theorem \ref{thm:dr} it holds that $Y^*_{DR}(\mathcal{O})=Y_{DR}(\mathcal{O})$.
	\end{Lemma}

	\begin{proof}
		As the first two arguments are the same, we only further investigate the integral.
		
		\begin{equation*}
		\begin{split}
			\int_{-\infty}^{Y}\frac{m(c,X;S)}{G^2(c\vert X)}dG(c \vert X) &= \int_{0}^{Y}\frac{m(c,X;S)}{G^2(c\vert X)}dG(c \vert X)\\
		...
		\begin{equation*}
		\begin{split}
		&\Lambda_G(c\vert X) = - \int_{0}^{c} \frac{G^\prime(t \vert X)}{G(t \vert X)}dt\\
		\Leftrightarrow&\frac{d\Lambda_G(c\vert X)}{dc} = -\frac{G^\prime(c \vert X)}{G(c \vert X)}
		\end{split}
		\end{equation*}
	\end{proof}
	...
	Within each subgroup a KM estimator is calculated and then connect them to an overall estimator for the observations.%XXXX
	...
	Instead, the survival curve is estimated via a random survival forest procedure, as we already suggested for the censoring distribution.
	...
	\todo[inline]{add reference for choice of dropout}
	...
	After fitting the keras model with the training data, we evaluate its performance with the IPC weighted mean squared error.
	
	Then we perform 5-fold cross validation to choose a suitable penalization error $\eta$ from the sequence $(0, 0.001, 0.01, 0.1)$.
	...
	We compare the deep learning algorithms that use the transformed data to three different approaches: a cox model, a penalized cox model and a random forest model.
	
	As a first step, we transform the responses $Y=h(T)=I(T>t)$, with the prespecified time point $t$.
	
	The cox model, which is explained in further detail in section \ref{cox}, can be estimated in R using the function coxph from the package survival.
	We then create the corresponding survival curve using the function survfit.
	
	The second model, which is the penalized cox model, is very similar to the cox model.
	...
	These covariates are then used to fit a cox model in the same way as described earlier in this section.
	...
	We now compare the results using different methods for time to event estimation.
