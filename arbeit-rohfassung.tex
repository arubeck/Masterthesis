...
\begin{document}
	...
	\end{equation*}
	 ...
	 We define the conditional survivor functions for $T$ and $C$ as $S_0(u \vert X)=P(T>u \vert X)$ and $G_0(u \vert X)=P(C>u \vert X)$ respectively.
...
%	\todo[inline]{add sth about IPCW weights}
	...
	In this context doubly robust implies that the mapping requires a well approximation of either  $S(\cdot\vert X)$ or $G(\cdot\vert X)$, but not necessarily both.
