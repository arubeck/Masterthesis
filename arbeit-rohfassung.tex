...
\setlength{\parskip}{2pt} %XXX maybe change distance
...
\begin{document}
	...
	For analysis and prediction we often use two important functions: the survival function and the hazard function.
	The survival function $S(t) = P(T > t)$ specifies the distribution of the survival time $F(t) = 1 - S(t)$.
	It can for example be used to analyze and compare the time to event of two different groups.
	The survival function has three important properties: $S(t)$ is a non-increasing function, $S(0)=1$ and $S(\infty)=0$.
	...
	In contrast to the survival function $S(\cdot)$ the hazard function does not necessarily have any monotonic properties.
	...
	\todo[inline]{add sentence: learn parameter beta for example by gradient descent}
...
%	\todo[inline]{add sth about IPCW weights}
	...
	We now describe how the transformation of the responses $T$ described in Section \ref{trafo}, namely the Buckley-James and the doubly robust transformation, can be implemented in \texttt{R}.
	...
	\end{equation*}
	which equals the Buckley-James transformation given in equation (\ref{eq:bjtrafo}) with response $Y=h(T)=I(T>t)$ and the observations $\mathcal{O}(t)$.
	...
	\todo[inline]{Add dimension of weight vector (which is 25*15+15+15+1)}
	...
	\todo[inline]{explanatory variables in Tabelle genauer erklären}
