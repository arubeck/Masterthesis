#requires function definition given in ma_create-data.R
#cox ph procedure
#choose which covariates should be incorporated
cov.mod.select = function(obs, delta, covs){
  # Creating a survival data frame
  data.surv = data.frame(obs, delta, covs)
  
  # Doing a cross validation across lambda
  cvfit = cv.glmnet(as.matrix(covs), Surv(obs, delta), family = "cox")
...
cox.calc.exp = function(cox.mod, cov.vec.test, active.min = NULL, tau)
{
  ...
  if(is.null(active.min) | length(active.min) > 0){
    ...
    for(i in 1:nu.test){
      ...
      m1[i]= sum((log(time.used) * surv.diff)[time.used < tau] )
    }
  }
...
compare.methods.brier <- function(x_train, y_train, x_test, y_test, data_surv, ipcw_weights, time.point){
  ...
  for(j in 1:5){
    ...
    #make sure that the data set contains more observations than covariates
    if(dim(x_train)[1] > dim(x_train)[2]){
      ...
      pred.err.cox.cv[j] = mean(ipcw_weights * (y.brier - pred.cox)^2)
    }
    ...
    if(not.active == TRUE){
      ...
      pred.err.cox.pen.cv[j] = mean(ipcw_weights * (pred.cox.pen - y.brier)^2)
    }
    ...
    pred.err.rand.cv[j] = mean(ipcw_weights * (y.brier - surv.est.default)^2)
    print(j)
  }
  
  if(dim(x_train)[1]> dim(x_train)[2]){
    object.return = list(pred.err.cox.pen = mean(pred.err.cox.pen.cv), pred.err.cox = mean(pred.err.cox.cv), pred.err.rand=mean(pred.err.rand.cv))}
...
#vergleichsmethoden?
#res.coxph <- coxph(Surv(time, status) ~ ., data=covs)

#L1 penalized cox ph
#opt.l1coxph <- optL1(Surv(time, status), penalized = covs, fold = 5)

#random survival forest
#rsf <- ranger(Surv(time = time, event = status) ~ ., data = covs)

#deepsurv algorithm
#the following function requires install_pycox(), install_keras() and install_torch() to be called first
#deep.surv <- deepsurv(Surv(time = time, event = status) ~ ., data = covs)
