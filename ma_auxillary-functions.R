...
parameter_RegularTreeKM <- function(obs, delta, covs, tau=NULL, quant=0.05){
  ...
  tem=SurvTreeEstimate_G(obs=obs,delta=delta,covs = covs, tau = tau, quant = quant)
  # Calculating the censoring distribution
  surC_rf=as.matrix(subset(tem, select=-(1:3)))
...
parameter_BJBrier <- function(obs, delta, covs, time.point){
  ...
  # Calculating the conditional expectation    
  m1 = BrierRandomForest_m(obs=obs,delta=delta,covs=covs, time.point=time.point)
