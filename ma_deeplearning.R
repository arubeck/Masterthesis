#one simulation of keras deep learning algorithm
one.sim.dl.dr = function(x_train, y_train.dr, x_test, y_test, data_surv, ipcw_weights, time.point){
  ...
  for(j in 1:5){
    ...
    pred.err.dl[j] = mean(ipcw_weights * (y.brier - predict)^2)
    ...
    for(l in 1:length(reg.par.used)){
      ...
      for (i in 1:k) {
        ...
        all_scores <-  c(all_scores, results[2]) #c(all_scores, results$mean_squared_error) ## <- hier ist der fehler
      }
    ...
    pred.err.dl.cv[j] = mean(ipcw_weights * (y.brier - predict.cv)^2)
  }
  
  return(list(pred.err.dl.cv = mean(pred.err.dl.cv), reg.par.final = reg.par.final, pred.err.dl = mean(pred.err.dl)))
}

one.sim.dl.bj = function(x_train, y_train.bj, x_test, y_test, data_surv, ipcw_weights, time.point){
  pred.err.dl = rep(NA, 5)
  pred.err.dl.cv = rep(NA, 5)
  reg.par.final = rep(NA, 5)
